...
def main_screen():
    ...
    bg_img = ImageTk.PhotoImage(Image.open(r"Requirements\bg2_.png"))
    ...
    image2 = load2.resize((100, 30), Image.ANTIALIAS)
    ...
    bg_label = Label(screen, image=bg_img).place(x=0, y=0, relwidth=1)
    Label(screen, image=render, bg="#576E8D", relief=GROOVE, bd=0).place(x=0, y=0, relwidth=1)
    Button(screen, image=render3, command=new_Records, bg="#BDC9D9", relief=SOLID, bd=1).place(x=230, y=150)
    Button(screen, image=render1, command=previous_Records, bg="#E1E4EB", relief=SOLID, bd=1).place(x=230, y=250)
    Button(screen, image=render2, command=screen.quit, bg="#EAEDF2", bd=0, relief=SUNKEN).place(x=315, y=430)

    Button(screen, image=render6, command=manage, relief=SOLID, bd=1).place(x=275, y=345)
...
background='darkblue', foreground='white', borderwidth=2)
    ...
    conn = sqlite3.connect(r"Database\ExpenseTracker.db")
    cur = conn.cursor()
    submit_btn = Button(new_screen, text="Add Record to Database", command=lambda: submit(_list)).grid(row=5, column=0,
                                                                                                       columnspan=2,
                                                                                                       pady=10, padx=10,
                                                                                                       ipadx=100)
    conn.commit()
